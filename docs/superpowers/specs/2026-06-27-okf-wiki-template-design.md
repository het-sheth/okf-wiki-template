...
faithful, **strict Google OKF v0.1**-conformant reference implementation of the Markdown-canonical
LLM-wiki pattern Het has been using across `education-wiki`, `ios-dev-wiki`, `voice-ai-wiki`, and
`course-wiki`.
...
| Conformance | **Strict OKF v0.1** (not Het's existing dialect). |
...
okf-wiki-template/
  ...
  wiki/getting-started/
    index.md           NO frontmatter — OKF directory listing (intro prose + links)
...
### 2. `index.md`: reserved, no-frontmatter directory listing
- `index.md` carries **no frontmatter**.
- The topic landing page is generated by `build.mjs` from the concept files in that directory
  (reading each concept's `title` + `description`); `index.md`, if present, supplies intro prose
  that renders above the generated listing.
- The old `check` rule "index.md must have `type: topic`" is **removed**; replaced with "index.md
  must have **no** frontmatter" (per OKF reserved-file rules).
...
- Strict per-path `type` validation (`source`/`topic`-equivalent/`concept`/`pattern`/`worked-example`/`log`),
  which is *stricter* than OKF (OKF only requires the field to be non-empty).
...
- **`README.md`** (human): one-paragraph "what is OKF" + link to Google's spec; "How to use this
  ...
  install` → `npm run check`); the optional-ingest section with its one-line removal; the
  build/check/test loop; a note that this is strict OKF (dialect-free) and why.

## Testing (the pass/fail signal)

- `npm run check` on the bundled example produces **zero violations**.
- `npm run build` regenerates `site/` and produces valid HTML for the example.
...
None blocking. Migration of existing wikis to match the strict template is explicitly deferred.
