...
// SHA256SUMS is `<hex>  <name>` per line, optionally `*name` for binary mode. Match the line
// that names THIS asset rather than trusting position: a release listing more than one asset
// would otherwise verify the tarball against some other file's digest and always mismatch.
// Returns null when the file has no entry for `name`, which callers must treat as unverifiable.
export function expectedSum(sums, name) {
  const lines = String(sums).split('\n').map((l) => l.trim()).filter(Boolean);
  const named = lines.find((l) => l.split(/\s+/).slice(1).join(' ').replace(/^\*/, '') === name);
  if (named) return named.split(/\s+/)[0];
  // A single-entry file with no recognizable name is still unambiguous.
  if (lines.length === 1 && lines[0].split(/\s+/).length === 1) return lines[0];
  return null;
}
